...
public class BlokusPresenter {
  ...
  public interface View {
    ...
    void setGameBoard(String yourPlayerId, List<String> playerList, String bitmap);
  ...
  public void updateUI(UpdateUI updateUI) {
    ...
    if (updateUI.getState().isEmpty()
        && yourPlayerId == Ordering.<String> natural().min(updateUI.getPlayerIds())) {
      sendInitialMove(updateUI.getPlayerIds());
      return;
    }

    String currentPlayerId = updateUI.getPlayerIds().get(
        (updateUI.getPlayerIds().indexOf(updateUI.getLastMovePlayerId()) + 1)
            % updateUI.getPlayerIds().size());
    currentState = BlokusState.getStateFromApiState(updateUI.getState(), currentPlayerId);
  ...
  private void updateGameBoard(String yourPlayerId, List<String> playerList, String bitmapStr) {
  ...
  public void makeMove(Piece piece, Point pos) {
    ...
    if (canFit) {
      ...
      /**
       * Do I really need to do this? Won't container send the updated state
       * back to me?
       */
